...
/** Clamped step along the tier ladder. Indexing ORDER directly returns
 *  `Tier | undefined` under noUncheckedIndexedAccess; this keeps it a Tier. */
function step(from: Tier, by: number): Tier {
  const i = Math.min(Math.max(ORDER.indexOf(from) + by, 0), ORDER.length - 1);
  return ORDER[i] ?? "standard";
}
...
export function computeTier(input: {
  ...
  if (knownOfferRate !== null) {
    ...
    if (ORDER.indexOf(byRate) > ORDER.indexOf(tier)) {
      tier = step(tier, 1); // at most one step
    }
  }
